...
CREATE TABLE empresarios_emprendimiento (
    ...
    cap_prod_mensual VARCHAR(5) NULL,
    porc_prod_planta VARCHAR(5) NULL,
    porc_prod_maquila VARCHAR(5) NULL,

    tiene_puntos_venta TINYINT NOT NULL DEFAULT 0,
    num_puntos_ventas VARCHAR(5) NULL,
    ...
    CONSTRAINT fk_empresarios_emprendimiento_empresario
        FOREIGN KEY (empresario_id)
        REFERENCES empresarios(id)
        ON DELETE CASCADE,

    CONSTRAINT fk_empresarios_emprendimiento_actividad
        FOREIGN KEY (actividad_id)
        REFERENCES actividades_pnte(id)
        ON DELETE CASCADE
);