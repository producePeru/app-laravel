...
class FormalizationController extends Controller
{
    ...
    public function allAsesorias()
    {
        ...
        ->through(function($item) {
            ...
            return [
                ...
                'sol_genero' => $item->person ? $item->person->gender : null,
    ...
    public function allFormalizations()
    {
        Carbon::setLocale('es');

        $data = Formalization10::with('categories', 'acreated', 'supervisor')
        ->orderBy('created_at', 'desc')
        ->where('status', 1)
        ->paginate(20)
        ->through(function($item) {
    ...
    public function formalizationsAll()
    {
        $data = FormFormalization::orderBy('created_at', 'desc')->get();
        return response()->json(['data' => $data]);
    }
}
