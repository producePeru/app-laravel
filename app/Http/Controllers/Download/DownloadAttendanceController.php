    ...
    public function exportAttendance(Request $request)
    {
        try {
            ...
            $actividades = ActividadPnte::with([
                ...
                ->select([
                    'id', 'unidad', 'mes', 'fechas', 'cantidad_dias',
                    'tipo_actividad_id', 'nombre_actividad_id', 'tema',
                    'region', 'provincia', 'distrito', 'lugar',
                    'entidad_organizadora', 'entidad_aliada', 'representante_id',
                    'requiere_pasaje', 'monto_gasto', 'mypes_beneficiadas',
                    'modalidad_id', 'total_participantes', 'total_asesorias',
                    'total_formalizaciones', 'slug', 'cancelado', 'reprogramado',
                    'registrado_por_id', 'tipo_mercado', 'tipo_gestion', 'created_at',
                ])
                ->addSelect([
            ...
            foreach ($actividades as $index => $item) {
                ...
                } elseif ($fechaMax && Carbon::parse($fechaMax)->lt($today)) {
                ...
                $representante = $item->representante
                    ? strtoupper($item->representante->lastname.' '.$item->representante->middlename.', '.$item->representante->name)
                    : null;

                $registradoPor = $item->registradoPor
                    ? strtoupper($item->registradoPor->name.' '.$item->registradoPor->lastname.' '.$item->registradoPor->middlename)
                    : null;

                $rows[] = [
                    $index + 1,
                    match ($item->unidad) {
                        1 => 'UGO', 2 => 'UGSE', 3 => 'UGSECO', default => '',
                    },
                    $mesFmt,
                    $fechaMinFmt,
                    $fechaMaxFmt,
                    ...
                    $item->tipo_mercado === null ? null : ($item->tipo_mercado == 1 ? 'MAYORISTA' : 'MINORISTA'),
                    $item->tipo_gestion === null ? null : ($item->tipo_gestion == 1 ? 'MUNICIPAL' : 'PRIVADA'),
                    $item->inscritos > 0 ? 'CON LISTA' : 'SIN LISTA',
                    $item->inscritos ?? 0,
            ...
            \Log::error('Error exportAttendance: '.$e->getMessage(), [
                'file' => $e->getFile(),
                'line' => $e->getLine(),
