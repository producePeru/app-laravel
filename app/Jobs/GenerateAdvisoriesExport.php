...
class GenerateAdvisoriesExport implements ShouldQueue
{
  ...
  public function __construct(array $filters, string $filename, int $userId, string $type = 'xlsx', ?string $email = null)
  {
    ...
    $this->userId = $userId;
    $this->type = $type;
    $this->email = $email ?? 'digitalizacion.pnte@gmail.com'; // valor por defecto si no viene
  }

  public function handle(AdvisoriesExportService $service)
  {
    try {
      ...
      Log::error('Error generando export: ' . $e->getMessage(), ['filters' => $this->filters]);
      throw $e;
    }
  }
}
